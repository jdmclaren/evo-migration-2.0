...
%      end
    ...
    if zug_opt == 1 %
        ...
        for iz = 1:n_zugs % add one to allow parfor loop 
            ...
            allel_zg_new_t1(i_dep,iz) = succ_al_zgs(i_parnt_1(i_dep),r_t1,1,iz);
            allel_sp_new_t1(i_dep,iz) = succ_al_sps(i_parnt_1(i_dep),r_t1,1,iz);
            allel_zg_new_t2(i_dep,iz) = succ_al_zgs(i_parnt_2(i_dep),r_t2,1,iz);
            allel_sp_new_t2(i_dep,iz) = succ_al_sps(i_parnt_2(i_dep),r_t2,1,iz);    
        end
        ...
        if n_loci > 1
            ...
            for iz = 1:n_zugs % add one to allow parfor loop 
                % (won't run anyway if n_zug == 0)
                allel_zg_new_m1(i_dep,iz) = succ_al_zgs(i_parnt_1(i_dep),r_t1,2,iz);
                allel_sp_new_m1(i_dep,iz) = succ_al_sps(i_parnt_1(i_dep),r_t1,2,iz);
                allel_zg_new_m2(i_dep,iz) = succ_al_zgs(i_parnt_2(i_dep),r_t2,2,iz);
                allel_sp_new_m2(i_dep,iz) = succ_al_sps(i_parnt_2(i_dep),r_t2,2,iz);    
            end
