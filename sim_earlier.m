function [successful, succ_wrmp, succs_DepPs, succs_ArrPs, succs_DepArrPs, ...
  ...
  simul_comp_rtes_genl ...
    ...
    frc_cmp_cst,mnt_TW_comp_wtr,max_wsp_dep,max_wsp_cst,hrzn_opt,hrzn_grid_res,fl_alt_lev)
...
%         arr_EU = false_vec;
        ...
        while sum(~finished) > 0 % 0.001*N_inds
           ...
           %  end
            ...
            th_wl_s = theta(idx_wants_to_lnd);
            ll_wl_s = ~near_dateline*llamda(idx_wants_to_lnd) + ...
                        near_dateline*mod(llamda(idx_wants_to_lnd),2*pi);   
            ...
            while any(~fl_hs_done) 
                ...
                % Lat and *1/cos(Lat) degs Lon 
                   ...
                   for i_nl = 1:n_to_land % par
                       ...
                       th_wl_i = th_wl_s(idx_nl);
                       ll_wl_i = ll_wl_s(idx_nl);
                       ...
                       % idx_iow = idx_not_yet_landed(idx_nl);
                        ...
                        if ~hrzn_opt % quadr decreasing chance detectn with dist
                            ...
                            sees_cst = find(d_SeeCst_2s < d2_hrzn_land ...
                                & rand(size(d_SeeCst_2s)) <  0.25); % Inf); %
...
%                 try
                         ...
                         if n_sees > 0 
                             ...
                             th_wl_s(idx_nl) = poss_lt_land(idx_sees);
                             ll_wl_s(idx_nl) = poss_ln_land(idx_sees);
                             ll_wl_s(idx_nl) = shiftAnglesFromMinus180To180(ll_wl_s(idx_nl)*rad2deg)*deg2rad;
                             ...
                             over_wtr_hs(idx_wants_to_lnd(idx_nl), ...
                                 curr_fls(i_nl):end) = false;

                             Lat_hs(idx_wants_to_lnd(idx_nl), ...
                                 curr_fls(i_nl):end) = th_wl_s(idx_nl);
                             Lon_hs(idx_wants_to_lnd(idx_nl), ...
                                 curr_fls(i_nl):end) = ll_wl_s(idx_nl);
                             % catch
                             %     keyboard
                             % end
                             ...
                             % idx_not_yet_landed = idx_not_yet_landed([1:idx_nl-1 idx_nl+1:end]);
                                ...
                                if rem_fls(i_nl) > 0
                                    ...
                                    th_wl_s(idx_nl) = Lat_hs(idx_wants_to_lnd(idx_nl), ...
                                     curr_fls(i_nl));
                                    ll_wl_s(idx_nl) = Lon_hs(idx_wants_to_lnd(idx_nl), ...
                                     curr_fls(i_nl));   
                                    catch
                                        keyboard
                                    end
           ...
           fl_hrs = curr_fl_hs;
            ...
            over_water = false_vec;
            over_water(rem_fl_hs == 0) = true;
