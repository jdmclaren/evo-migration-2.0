function [lat_bs_deps, lon_bs_deps,idxSubPoly] = ...
    initialize_locs(N_inds, Lon_poly, Lat_poly,  near_idx_cst, d_cst_vec, ...
    require_land,hi_elevs, barrens, poor_stops, ok_veg_breed, near_dateline)
...
%         end
        ...
        % if ~near_dateline
        %     idx_cst = near_idx_cst(lond_i, latd_i);
        % else
            idx_cst = near_idx_cst(mod(lond_i, 360), latd_i);
        % end
        d_cst_ds = d_cst_vec(idx_cst);

         % over_water(~finished) = d_cst_ds<0;

         viable_Loc = d_cst_ds>=0 & ~is_barrn & ~is_high_elev ...
             & ~is_poor_stop & is_ok_veg_br & inPoly;    

         % viable_Loc(lond_i > 10 & lond_i < 30 & latd_i > 50 & latd_i < 70) = true;
