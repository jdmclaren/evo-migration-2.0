...
% here we assume two alleles on diff chromosome 
% but with orientn and signp cues on same chrms
...
all_allels(:,:,1,1) = [all_allel_t1 all_allel_t2];  
T1s = all_allels(:,1,1);
T2s = all_allels(:,2,1);    
...
%     avgd = (M1s == M2s & T1s == T2s);
    ...
    % combine for determination std's and disp
    % These 'technical vars are not double-chrms based, but 
    % will retain as loci based

    avgd = avgd_t | avgd_m;
    incl_1 = avgd | (inh_1t | inh_1m);
    incl_2 = avgd | (inh_2t | inh_2m);
...
if ~isinf(inher_head_kappa(1))
    ...
    allel_hd_new_t1 = vmrand(allel_hd_new_t1, ...
            inher_head_kappa,[N_inds,1]);
    allel_hd_new_t2 = vmrand(allel_hd_new_t2, ...
            inher_head_kappa,[N_inds,1]);  

    allel_hd_new_m1 = vmrand(allel_hd_new_m1, ...
            inher_head_kappa,[N_inds,1]);
    allel_hd_new_m2 = vmrand(allel_hd_new_m2, ...
            inher_head_kappa,[N_inds,1]);  

    if zug_opt == 1 
        allel_zg_new_t1 = vmrand(allel_zg_new_t1, ...
                inher_head_kappa,[N_inds,1]);
        allel_zg_new_t2 = vmrand(allel_zg_new_t2, ...
                inher_head_kappa,[N_inds,1]);
        allel_zg_new_m1 = vmrand(allel_zg_new_m1, ...
                inher_head_kappa,[N_inds,1]);
        allel_zg_new_m2 = vmrand(allel_zg_new_m2, ...
                inher_head_kappa,[N_inds,1]);  
    end
...
if zug_signp == 1 || zug_signp ==  2 % incln and decln are circular var 
        ...
        for iz = 1:n_zugs
            ...
            allel_sp_new_t1 = min(vmrand(allel_sp_new_t1(:,iz), ...
                   inher_signp_kappa, [N_inds 1]),pi/2);

            allel_sp_new_t2 = min(vmrand(allel_sp_new_t2(:,iz), ...
                   inher_signp_kappa, [N_inds 1]),pi/2);

            allel_sp_new_m1 = min(vmrand(allel_sp_new_m1(:,iz), ...
                   inher_signp_kappa, [N_inds 1]),pi/2);

            allel_sp_new_m2 = min(vmrand(allel_sp_new_m2(:,iz), ...
                   inher_signp_kappa, [N_inds 1]),pi/2);
    ...
    for iz = 1:n_zugs
        allel_sp_new_t1 = allel_sp_new_t1(:,iz).* ...
            (1 + randn(N_inds,1).*std_inher_signp); 

        allel_sp_new_t2 = allel_sp_new_t2(:,iz).* ...
            (1 + randn(N_inds,1).*std_inher_signp);  
        
        allel_sp_new_m1 = allel_sp_new_m1(:,iz).* ...
            (1 + randn(N_inds,1).*std_inher_signp); 

        allel_sp_new_m2 = allel_sp_new_m2(:,iz).* ...
            (1 + randn(N_inds,1).*std_inher_signp);   
    end
...
allel_heads(:,:,1) = [allel_hd_new_t1 allel_hd_new_t2]; % [allel_hd_new_1 allel_hd_new_2];  
allel_heads(:,:,2) = [allel_hd_new_m1 allel_hd_new_m2]; % [allel_hd_new_1 allel_hd_new_2];  

if zug_opt
    allel_zugs(:,:,1,:) = [allel_zg_new_t1 allel_zg_new_t2];  
    allel_zugs(:,:,2,:) = [allel_zg_new_m1 allel_zg_new_m2];  
    allel_sps(:,:,1,:) = [allel_sp_new_t1 allel_sp_new_t2];  
    allel_sps(:,:,2,:) = [allel_sp_new_m1 allel_sp_new_m2]; 
end
...
% With two chromas, we have (for now) two genes each 
% % coded for 360-deg headings
