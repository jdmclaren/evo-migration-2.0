...
chck_ahd = can_dtr | (dep_wtr_TWs & ~over_water);
...
if ~isempty(idx_chck_ahd)
    ...
    llamda(idx_chck_ahd),alpha(idx_chck_ahd),N_S_migr,0,zero_chk,zero_chk, ...
    ...
    dtr_ahd= idx_chck_ahd(ahead_wtr | ahd_hi_elev);
...
%     try
           ...
           parfor i_dtr = 1:numel(dtr_ahd) % 
                ...
                [ahead_theta_clws, ahead_llamda_clws] = update_loc(...
                    ...
                    ll_dtr(i_dtr,:),alpha_bins_clws,N_S_migr,include_wind, ...
                ...
                [ahead_theta_anticlws, ahead_llamda_anticlws] = update_loc(...
                    ones_dtr,radAheadDtrCst,th_dtr(i_dtr,:), ...
                    ll_dtr(i_dtr,:),alpha_bins_anticlws,N_S_migr,include_wind, ...
                    uw_dtr(i_dtr,:),vw_dtr(i_dtr,:), ...
                    star_cmp_flag,decl_dtr(i_dtr,:),date_dtr(i_dtr,:),geomagn_model);
...
%                            del_alph_dtr(dtr_ahd(i_dtr)) = 0;
                          ...
                          if dep_wtr_TWs % no viewable coast, stick to heading, i.e. no detour

                           % if not already stopped, do so
                            if cumul_flts(dtr_ahd(i_dtr)) > 0

                                n_fl_step(dtr_ahd(i_dtr)) = n_fl_step(dtr_ahd(i_dtr)) + stop_durn;
    ...
    %                             day_step(stops_barrier) = day_step(stops_barrier) + stop_durn;
               ...
               while any(tail_w(stops_barrier) < 0)
                    ...
                    u_wind(hws), v_wind(hws),N_S_migr);
