...
overshot_Lon = ...
 (pos_d_0_lon_lim & (mod(llamda,2*pi) - arr_lon_lim > lst_Ln_wdth)) | ... > arrLon_width2)) | ...
 (~pos_d_0_lon_lim & (arr_lon_lim - mod(llamda,2*pi) > lst_Ln_wdth)); % ... > arrLon_width2));
        
if max_Lat_Dep > 0
    overshot_Lat =  (min_Lat_Arr - theta) > lst_Lt_wdth | ... arrLat_width2 | ... 
            (theta - max_Lat_Dep) > lst_Lt_wdth; % depLat_width2;
else
    overshot_Lat =  (max_Lat_Arr - theta) < -lst_Lt_wdth | ... -arrLat_width2 | ... 
            (theta - min_Lat_Dep) < -lst_Lt_wdth; % -depLat_width2;
end
...
finished = finished | lost | ~survived | is_late;

%% then check for new arrival
     ...
     for iSt = 1:nStop_polys
         % check if stopped in obligatory staging region
            ...
            passed_Stops(~finished,iSt) =  passed_Stops(~finished,iSt) | ...
                 any(((req_land_stop & ~over_wtr_hs(~finished,:)) | ~req_land_stop) &  ...
                inpolygon(Lon_hs(~finished,:),Lat_hs(~finished,:), ...
...
%               end
     ...
     if ~isempty(new_stop) > 0
          for inw = 1:numel(new_stop)
              idx_new = find( ...
                 ((req_land_stop & ~over_wtr_hs(new_stop(inw),1:9)) |  ~req_land_stop) & ...
                 inpolygon(Lon_hs(new_stop(inw),1:9),Lat_hs(new_stop(inw),1:9), ...
...
%      arrived(~finished) = any(((req_land_arr & ~over_wtr_hs(~finished,:)) |  ~req_land_arr) & ...
...
try
     if ~near_dateline
         arrived(~finished) = ...
           any(((req_land_arr & ~over_wtr_hs(~finished,:)) |  ~req_land_arr) & ...
            inpolygon(Lon_hs(~finished,:),Lat_hs(~finished,:), ...
              Lon_arr_poly',Lat_arr_poly'),2); 
     else
         arrived(~finished) = ...
           any(((req_land_arr & ~over_wtr_hs(~finished,:)) |  ~req_land_arr) & ...
            inpolygon(mod(Lon_hs(~finished,:),2*pi),Lat_hs(~finished,:), ...
              Lon_arr_poly',Lat_arr_poly'),2); 
     end
catch
    keyboard
...
%  end  % arrived(~finished) | ... {iAr}
 new_arr = find(~finished & arrived & ~curr_arr);
 ...
 if ~isempty(new_arr) > 0
      for inw = 1:numel(new_arr)
         if ~near_dateline
             idx_new = find( ...
                 ((req_land_arr & ~over_wtr_hs(new_arr(inw),:)) | ~req_land_arr) & ...
                 inpolygon(Lon_hs(new_arr(inw),:),Lat_hs(new_arr(inw),:), ...
                 Lon_arr_poly',Lat_arr_poly'),1,'first'); % 'last');
         else
             idx_new = find( ...
                 ((req_land_arr & ~over_wtr_hs(new_arr(inw),:)) | ~req_land_arr) & ...
                 inpolygon(mod(Lon_hs(new_arr(inw),:),2*pi),Lat_hs(new_arr(inw),:), ...
                 Lon_arr_poly',Lat_arr_poly'),1,'first'); %,'last');
         end
         % try
          llamda(new_arr(inw)) = Lon_hs(new_arr(inw),idx_new);
          theta(new_arr(inw)) = Lat_hs(new_arr(inw),idx_new);

          Lon_hs(new_arr(inw),idx_new+1:end) = NaN;
          Lat_hs(new_arr(inw),idx_new+1:end) = NaN;

          if isnan(llamda(new_arr(inw)))

              keyboard

          end

          llnw = mod(llamda(new_arr(inw))*180/pi,360);
          lltw = theta(new_arr(inw))*180/pi;

          % if llnw > 170.5 && llnw < 176 && lltw > -21 && lltw < -13
          %     keyboard
          % end
...
% once again update finsihed status

finished = finished | arrived;
