...
%      all_allels(lon_bs_deps-min_lon_dep<d_lon/3,:,2) = 1;
     ...
     if n_loci == 1
         ...
         if rndm_init_allels
            ...
            all_allels(:,1:2) = randi(2,[N_inds,2])-1; %

         elseif unif_allels % no dominance always averaged

              all_allels(:,1) = one_vec; % randi(2,[N_inds,1])-1; % 1*(lat_bs_deps<=-41); % > 61); % 
              all_allels(:,2) = one_vec; % randi(2,[N_inds,1])-1; % 1*(lat_bs_deps<=-41); %; 

         elseif ~near_dateline % one dom allele

             % trochilus gene homzyg (dom) below 61 and (rec) above 63 N
             ...
             all_allels = randi(2,[N_inds,2])-1; % 1*(lat_bs_deps>=-41); % 

             % trochilus gene on 1st element last index
             all_allels(Troch_hom,:) = 0;
             all_allels(Acred_hom,:) = 1;

         else

             all_allels = randi(2,[N_inds,2])-1; % 1*(lat_bs_
             all_allels(:,1) = 1*(lat_bs_deps > -40); % randi(2,[N_inds,1])-1; % > 61); % 
             all_allels(:,2) = 1*(lat_bs_deps <= -42);
            

         end

        if EW_dom % Dom allele is first allel
        ...
        else % Dom allel is 2nd but homozygous zero (i.e., dominance is ignored)
     ...
     else % always 2nd allele dom (need one to be since only one Chr)
         ...
         elseif ~near_dateline 
             ...
             % second alleles
             % all_allels(:,1,1) = 1*(lat_bs_deps < 61); % randi(2,[N_inds,1])-1; % > 61); % 
             % all_allels(:,2,1) = randi(2,[N_inds,1])-1; % 1*(lat_bs_deps>=-41); % 
             % all_allels(lon_bs_deps >= 35,2,1) = 0;
             % all_allels(lon_bs_deps <= 6,2,1) = 1; %1*(lat_bs_deps<41); % 
             % % all_allels(:,1,2) = 1*(lat_bs_deps >= 61); % randi(2,[N_inds,1])-1; %   > 61
             % all_allels(:,2,2) = randi(2,[N_inds,1])-1; %1*(lat_bs_deps<41); % 
             % all_allels(lon_bs_deps >= 35,2,2) = 1;
             % all_allels(lon_bs_deps <= 6,2,2) = 0; %1*(lat_bs_deps<41); % 

         else % L T Cuckoo
         % 
             all_allels(:,1,1) = 1*(lat_bs_deps > -40); % randi(2,[N_inds,1])-1; % > 61); % 
             all_allels(:,2,1) = randi(2,[N_inds,1])-1; % 1*(lat_bs_deps>=-41); % 
             all_allels(:,1,2) = 1*(lat_bs_deps <= -42); % randi(2,[N_inds,1])-1; %   > 61
             all_allels(:,2,2) = randi(2,[N_inds,1])-1; %1*(lat_bs_deps<41); % 
         % 
         end
...
if zug_opt == 1
    ...
    if ~isinf(compass_kappa_incl)
         rand_magn_a = vmrand(0, compass_kappa_incl, [N_inds 2]); %      
    else
         rand_magn_a = [zero_vec zero_vec];
    end
    ...
    if zug_signp == 0                   
        ...
        allel_sps =  NaN*ones(N_inds,2,n_zugs);
%                     zug_signps = sort(zug_signps,2,'descend');
    ...
    elseif zug_signp == 1
        ...
        if  zgknk_req_in_stop(1) == 1 || zgknk_req_stop_warm % (zug_opt == 1 &&  n_zugs > 0) %  || 
            ...
            allel_sps(:,:,1) = [incln_a incln_a]; % + rand(N_inds,n_zugs).*(incln_0 - incln_a);
            
        else
            
            allel_sps(:,:,1) = incln_a  + rand(N_inds,2,n_zugs).*(incln_0 - incln_a);
              
        end
        
        if n_zugs > 1
            allel_sps = sort(allel_sps,3,'descend'); 
        end
...
%                 zug_signps = rand(N_inds,n_zugs).*(1-tot_B_a./tot_inten_0); 
        ...
        if zgknk_req_in_stop(1) == 1 || zgknk_req_stop_warm % (zug_opt == 1 &&  n_zugs > 0) %  || 
             ...
             allel_sps(:,:,1) = decln_a + rand(N_inds,2,n_zugs).*(decln_0 - decln_a); 
        ...
        if n_zugs > 1
            if mean(decln_0) > mean(decln_a)
                ...
                allel_sps = sort(allel_sps,3,'descend');   
                 ...
                 allel_sps = sort(allel_sps,3,'ascend');                          
    ...
    elseif zug_signp == 3
        ...
        if  zgknk_req_in_stop(1) == 1 || zgknk_req_stop_warm % (zug_opt == 1 &&  n_zugs > 0) %  || 
            ...
            allel_sps = tot_B_a  + rand(N_inds,2,n_zugs).*(tot_inten_0 - tot_B_a); % Bza./tot_B_a + rand(N_inds,n_zugs).*(vt_inten_0 - Bza./tot_B_a);
    ...
    elseif zug_signp == 4  
        ...
        if  zgknk_req_in_stop(1) == 1 || zgknk_req_stop_warm % (zug_opt == 1 &&  n_zugs > 0) %  || 
                ...
                allel_sps(:,:,1) = vt_inten_a + rand(N_inds,2,n_zugs).*(vt_inten_0 - vt_inten_a);
        end
%                 zug_signps = Bza./tot_B_a + rand(N_inds,n_zugs).*(vt_inten_0 - Bza./tot_B_a);
        if n_zugs > 1
            allel_sps = sort(zug_signps,3,'descend');
        end
    ...
    elseif zug_signp == 5  
        ...
        if  zgknk_req_in_stop(1) == 1 || zgknk_req_stop_warm % (zug_opt == 1 &&  n_zugs > 0) %  || 
            ...
            allel_sps(:,:,1) = hz_inten_a + rand(N_inds,2,n_zugs).*(hz_inten_0 - hz_inten_a);
...
inher_offs = -pi/2 + pi*rand(N_inds*2,1);
...
if abs(circ_mean(mn_angs_arr)) > pi/2 % S
    ...
    allel_heads(:,1) = mod(mn_angs_arr + inher_offs(1:N_inds)+pi,2*pi)-pi;
    allel_heads(:,2) = mod(mn_angs_arr + inher_offs(N_inds+1:2*N_inds)+pi,2*pi)-pi;
    N_S_migr = false;
end
...
if zug_opt
   ...
   if zgknk_req_in_stop(1) == 1  || zgknk_req_stop_warm %% (zug_opt == 1 &&  n_zugs > 0) %  || 
    ...
    %                                 [sun_az_stop,~] = SolarAzEl(dates,lat_bs_stops,lon_bs_stops,zero_vec);
                ...
                allel_zugs(:,1,iz) = randinterval(pi,pi+sign_mn_ang*pi/12,N_inds) ...
                    - (endog_comp == 1)*median(decln_stop); %
                allel_zugs(:,2,iz) = randinterval(pi,pi+sign_mn_ang*pi/12,N_inds) ...
                    - (endog_comp == 1)*median(decln_stop); %
    %                             zgkn_fun = zgkn_lots;
    ...
    %             allel_zugs(:,iz) = inher_heads(randperm(length(inher_heads)));
                allel_zugs(:,1,iz) = allel_heads(randperm(size(allel_heads,1)),1);
                allel_zugs(:,2,iz) = allel_heads(randperm(size(allel_heads,1)),2);
            end
    %     else
...
allel_heads(:,1) = -pi/2 + pi*rand(N_inds,1);
allel_heads(:,2) = allel_heads(:,1); % -pi/2 + pi*rand(N_inds,1);
% allel_heads(:,2) = -pi/2 + pi*rand(N_inds,1);

% if zug_opt

    if EW_dom % only magnitude of dirn inherited 
        % and E/W determined by genes
    ...
    else % 1 genes inherits signed direction 
        [inher_heads, zugkn_inher_heads, zug_signps] = ...
            det_phentyp(allel_heads,allel_zugs,allel_sps, ...
            idx_avgs,idx_1s,idx_2s,N_avgs,N_1s,N_2s,n_zugs,zug_signp,Dom_phen,n_heads);
    end
