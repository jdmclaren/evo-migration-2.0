...
try
     if ~near_dateline
         ...
         arrived(~finished) = ...
           any(((req_land_arr & ~over_wtr_fracs(~finished,:)) |  ~req_land_arr) & ...
            inpolygon(mod(llamdas_fracs(~finished,:),2*pi),thetas_fracs(~finished,:), ...
              Lon_arr_poly',Lat_arr_poly'),2); 
     end
catch
    keyboard
 ...
 if ~isempty(new_arr) > 0
      for inw = 1:numel(new_arr)
         if ~near_dateline
             ...
             idx_new = find( ...
                 ((req_land_arr & ~over_wtr_fracs(new_arr(inw),:)) | ~req_land_arr) & ...
                 inpolygon(mod(llamdas_fracs(new_arr(inw),:),2*pi),thetas_fracs(new_arr(inw),:), ...
                 Lon_arr_poly',Lat_arr_poly'),1,'last');
         end
%          try
...
%                 (SW & diff_Lons_SW > arrLon_width2);
 ...
 overshot_Lon = ...
     (pos_d_0_lon_lim & (mod(llamda,2*pi) - arr_lon_lim > arrLon_width2)) | ...
     (~pos_d_0_lon_lim & (arr_lon_lim - mod(llamda,2*pi) > arrLon_width2));
            
 if max_Lat_Dep > 0
     overshot_Lat =  (min_Lat_Arr - theta) > arrLat_width2 | ... 
                (theta - max_Lat_Dep) > depLat_width2;
 else
     overshot_Lat =  (max_Lat_Arr - theta) < -arrLat_width2 | ... 
                (theta - min_Lat_Dep) < -depLat_width2;
 end
