function [successful, succ_wrmp, succs_DepPs, succs_ArrPs, succs_DepArrPs, ...
    ...
    all_allels,allel_heads,allel_zugs,allel_sps,  ...
    all_inher_heads,all_zugkn_heads,all_geo_heads, all_mag_heads,all_sun_heads, ...
    all_coeffs, blats, blons,blat_hs, blon_hs,lat_bs_zugs,lon_bs_zugs, passed_Stops, ...
    arrived, survived, lost, stoppedAndArrived, init_allels, all_ys_allels, ...
  ...
  sim_mig_gene_phene ...
        (n_loci,n_chrs,geo_filnm,N_inds_sim,N_mig_Sims,N_wrmp_rnd, ...
        N_wrmp_bck,N_wrmp_1st,N_init_fact,init_hds_propn_lnd,start_date, ...
    ...
    min_ns_cst_hs,max_ns_cst_hs,min_ns_sgnp_hs, max_ns_sgnp_hs, ... % May 2022 added distinct refuelling at signpost
    ...
    maxDtrSteps,stopCstOpt,fracStopCst,fracStopCst_init,stpWtrFlt,stpovWtrFlt,stpovCstWtr, ...
    ...
    include_wind,wind_str,wind_form,weib_scale,weib_par,opt_stdy_swtch_ws, ...
    dep_TWs,dep_wtr_TWs,min_disp_d_dep_km,max_disp_d_dep_km,min_disp_d_arr_km,max_disp_d_arr_km, ...
    ...
    opt_same_loc_brd,accel_wrmp,arr_EU_opt,veg_mod_pal, ...
    match_init_Lons,n_heads,EW_dom,EW_avg,genSel,genSelWt,rndm_init_allels, ...
    unif_allels,comp_HWs_dep, comp_HWs_cst, comp_TWs_dep,comp_TWs_cst, ...
    frc_cmp_dep,mnt_TW_comp_wtr,max_wsp_dep,max_wsp_cst,hrs_wnd_updat,p_stdy_w_updt, ...
    hrzn_opt,hrzn_grid_res,fl_alt_lev,p_see_lnd_pixl,lst_Lt_wdth,lst_Ln_wdth)


% determine number of initial (warmup or spinup) simulations to coastlatparp
...
% max_degs_cst = d_stop_max/R_Earth_km*180/pi;
...
% arrLat_width2 = 0; % (max_Lat_Arr - min_Lat_Arr)/2;
min_Lon_Arr = nanmin(Lon_arr_poly);
max_Lon_Arr = nanmax(Lon_arr_poly);
% arrLon_width2 = 0; % (max_Lon_Arr - min_Lon_Arr)/2;

mean_arr_Lat = min_Lat_Arr + lst_Lt_wdth; % arrLat_width2;
mean_arr_Lon = min_Lon_Arr + lst_Ln_wdth; % arrLon_width2;

% in case polys are > 180 use simple addition
min_Lon_Dep = nanmin(Lon_dep_poly);
max_Lon_Dep = nanmax(Lon_dep_poly);
depLon_width2 = 0; % (max_Lon_Dep - min_Lon_Dep)/2;
...
% 
wide_EW_route = abs(mean_arr_Lon-mean_dep_Lon) > (max_Lat_Arr - min_Lat_Arr)/2; % lst_Ln_wdth; % arrLon_width2;
sign_mean_arr_dep_Lon = sign(mean_arr_Lon-mean_dep_Lon);

% now compute geo data and 
...
% assess if near dateline (if within 30 degs) to make close to coast work
% easily using Lat and Lon differences instead of more costly arc dist
if any(abs([min_Lon_Dep max_Lon_Dep min_Lon_Arr max_Lon_Arr ]) >  5*pi/6) %
        near_dateline = true;
else
        near_dateline = false;
end
...
[coastlat,coastlon, near_idx_cst, d_cst_vec, igrfcoefs, hi_elevs, barrens, ...
    poor_stops, ok_veg_breed, ndvi, veg_stp_md, veg_stp_95, ...
    veg_stp_sum_md, veg_stp_sum_95, thrsh_veg_stop_max, ...
...
if ~hrzn_opt
    ...
    max_d_hrzn = max(sqrt(d2_hrzn(:)));
    max_rad_stop_dawn = max_d_hrzn;

    hrzn_grid_res_rad = hrzn_grid_res*pi/180;

end

max_rad_st_dwn_2 = max_rad_stop_dawn^2;

% added horizn for lb cuckoos (sys = 12)
% hrzn_opt = mig_sys ==12;
...
mn_std_inh_hds = NaN(N_mig_Sims,1);
...
decln_0s = zeros(N_inds_sim,N_mig_Sims);
incln_0s = NaN(N_inds_sim,N_mig_Sims); 
intns_0s = NaN(N_inds_sim,N_mig_Sims);

all_ys_init_geo_heads = NaN(N_inds_sim,N_mig_Sims);
all_ys_inher_heads = NaN(N_inds_sim,N_mig_Sims);
all_ys_zugkn_heads = NaN(N_inds_sim,N_mig_Sims);
all_ys_zugkn_sps = NaN(N_inds_sim,N_mig_Sims);
...
all_lat_arr_EU = NaN(N_inds_sim,N_mig_Sims);
all_lon_arr_EU = NaN(N_inds_sim,N_mig_Sims);
all_ys_lat_fin = NaN(N_inds_sim,N_mig_Sims);
all_ys_lon_fin = NaN(N_inds_sim,N_mig_Sims);
all_lat_zg = NaN(N_inds_sim,N_mig_Sims);
all_lon_zg = NaN(N_inds_sim,N_mig_Sims);

% if ~isinf(n_chrs) 
    all_ys_allels = NaN(N_inds_sim,2,n_loci,N_mig_Sims);
% else % if Inf set n_loci == 1
%     all_ys_allels = NaN(N_inds_sim,2,n_loci,N_mig_Sims);
% end
...
%     prev_coeff_succ = []; 
    ...
    % isinf(n_chrs) % only genotype evolved 
    %     % (too many interactions so average parental traits, adding noise)
    %     all_allels = NaN; % ,min(n_loci,2+2*n_zugs))n_req;
    %     allel_heads = NaN; 
    %     allel_zugs = NaN; 
    %     allel_sps = NaN;        
    % else

    if n_chrs == 1 || isinf(n_chrs)
        % number of alleles is 2 per heading plus signposts
        all_allels = NaN(N_inds_sim,2,n_loci); % ,min(n_loci,2+2*n_zugs))n_req;
        allel_heads = NaN(N_inds_sim,2);
        allel_zugs = NaN(N_inds_sim,2,n_zugs);
        allel_sps = NaN(N_inds_sim,2,n_zugs);
    else % 2 headings (for now)
        all_allels = NaN(N_inds_sim,2,n_loci); % ,min(n_loci,2+2*n_zugs))n_req;
        allel_heads = NaN(N_inds_sim,2,n_loci);
        allel_zugs = NaN(N_inds_sim,2,n_loci,n_zugs);
        allel_sps = NaN(N_inds_sim,2,n_loci,n_zugs);
    end
...
%            end
        ...
        zero_24_vec = zeros(N_inds,24);
        one_vec = ones(N_inds,1);
        NaN_vec = NaN(N_inds,1);
        NaN_24_vec = NaN(N_inds,24);
        false_vec = false(N_inds,1);
        true_vec = true(N_inds,1);
        false_24_vec = NaN(N_inds,24);
...
%         date_jan_1 = datenum(curr_year,1,1);  
        ...
        if iWarmup <= 1 &&  iWarmup <= N_init_Sims && iYear < 2  
           ...
           if n_chrs == 1 || isinf(n_chrs)
               ...
               create_init_genes_traits 

           else

               create_init_genes_two_chrms 

           end
           % ##

           init_allels = all_allels;

        else % "iYear" or "iWarmup" > 1, use 'inherited' values init heads and any 'steering'
   
            if isinf(n_chrs) && n_loci == 1

                update_succ_traits 
                    
            elseif n_chrs ~= 2

               update_successful_genes_traits 

           else

               update_successful_genes_two_chrms
    ...
    if n_zugs > 0
        lat_bs_zugs = NaN(N_inds,n_zugs);
        lon_bs_zugs = NaN(N_inds,n_zugs);
    else
        lat_bs_zugs = NaN(N_inds,1);
        lon_bs_zugs = NaN(N_inds,1);          
    end
...
% compass & steering options
    ...
    % check if starts over land
         % over_wtr_hs_0 = ~island(location.latitude,location.longitude); % ,cst_accuracy);   
        
     idx_cst_0 = near_idx_cst(location.longitude,location.latitude);
    % end
    d_csts_0 = d_cst_vec(idx_cst_0);
    over_water_0 = d_csts_0 < 0;
...
% initialize location indices for elevation and barren grids
...
% new 2024 maintain adjusted headings whbefore over wtare crossings 
% while over water (e.g. endurance flights) 
        del_alpha_dep = zero_vec;
...
% North S hemisphere defined in create_init_genes_traits
        ...
        blat_hs = blats; % repmat(theta_0,[1 24]); % lat_bs_deps(1:N_inds,1);
        blon_hs = blons; % repmat(llamda_0,[1 24]); % lon_bs_deps(1:N_inds,1);;

        % set ref lon for time compensation
        ref_lon_tc = llamda_0;
        all_inher_heads = inher_heads;

        % assume no winds; otherwise, will be updated
        u_wind = zero_vec;
        v_wind = zero_vec;
        % effective wind speed after compensation
        uw_eff = zero_vec;
        vw_eff = zero_vec;
        tail_w = zero_vec;
        cross_w = zero_vec;
        % initialize vector of Weibull speeds and exp's
        weib_str_exp = weib_scale*one_vec;
        weib_exp = weib_par*one_vec;
...
% direction (helps flag lost migrants to speed up computation)
         ...
         arr_lon_lim = Wstwd.*mod(min_Lon_Arr,2*pi) + ...
                            Eastwd.*mod(max_Lon_Arr,2*pi);
...
% flag sign of original difference with lon limit
       ...
       % first check for detours
       if dtrCstOpt==1

           can_dtr = dtrCstOpt==1 & d_csts_0 > 0 & ...
               d_csts_0 < kmsAheadDtrCst;
        ...
        if sum(can_dtr) > 0 % this will update alpha
             determine_detours_barrier_crossings
        end

        % next check if water ahead (ignore wind)
  
       lnd_wtr_ahd = false_vec;

       [ahead_theta, ahead_llamda] = update_loc( ... 
        one_vec,hourly_del_Lat,theta, ... radAheadDtrCst/2
        llamda,alpha,N_S_migr, ...
        zero_vec,zero_vec, ...
        star_cmp_flag,decln,date_jul,geomagn_model);
    
       % lnd_wtr_ahd = island(ahead_theta*rad2deg, ahead_llamda*rad2deg) == 0;

        idx_ahd_cst = near_idx_cst(ahead_llamda*rad2deg,ahead_theta*rad2deg);
        % end
        d_ahd_csts = d_cst_vec(idx_ahd_cst);

        % ahead wtr for possible TW comp at coast (all begin over land)
        lnd_wtr_ahd = d_ahd_csts < 0;

        % u_wind, v_wind is wind leading to initial locn
        % needed for case of opt_stdy_swtch_ws 

        if include_wind

            % 2024 last argument is for new time step (o/w hourly winds)
            if wind_form == 1 % constant wind str

                [u_wind, v_wind] = det_stoch_wind(theta,wind_str,NaN,NaN,0,true); % d

            else % Weibull 
    
                % set opt_stdy_swtch_ws off for first flt step
                [u_wind, v_wind] = det_stoch_wind_weib(theta, ...
                    weib_str_exp,weib_exp, ...
                    false,NaN,NaN,true_vec,false,NaN); % d
                % scale wind to airsp (inverse)
                u_wind = wind_str*u_wind;
                v_wind = wind_str*v_wind;


            end

            % Determine tw and cw compts &
            % adjust headings for any wind compn, waiting for TWs @coast if required

            is_deps = true;
            select_adj_hds_TWs

            % adjust for comp at cst
            alpha = alpha + del_alpha_dep;
...
%         end
        ...
        Lat_hs = NaN_24_vec;
        Lon_hs = NaN_24_vec;
        % 2025 added hourly decln updating
        decln_hs = NaN_24_vec;
        ...
        if nDep_polys > 1
            ...
            max_cst_hs_bs = max_ns_cst_hs(idx_deps)'; 
            min_cst_hs_bs = min_ns_cst_hs(idx_deps)';              
            ...
            max_cst_hs_bs = max_ns_cst_hs*one_vec; 
            min_cst_hs_bs = min_ns_cst_hs*one_vec;  
        ...
        if N_S_migr
            ...
            all_geo_heads = alpha_0*180/pi;
            all_sun_az = sun_az_0*180/pi;
        end
...
%         arr_EU = false_vec;
        ...
        while sum(~finished) > 0 % 0.001*N_inds
            ...
            % check if initial step of endurance flight (then won't stop for visible land)

            init_endur = n_fl_step==1 & endur_flt; % false_vec; % 

            % initialize decile to land to 10th (last)
            land_frac = fracStopCst*one_vec;  
            ...
            % initialize final loc (for finsihed inds)
            % Lat_fins = theta;
            % Lon_fins = llamda;
            Lat_currs = theta(~finished);
            Lon_currs = llamda(~finished);
            Lat_curr_hs = NaN_24_vec(~finished); % Lat_hs(~finished,:);
            Lon_curr_hs = NaN_24_vec(~finished); % Lon_hs(~finished,:);
            Lat_hs(~finished,:) = NaN_24_vec(~finished,:);
            Lon_hs(~finished,:) = NaN_24_vec(~finished,:);
            % Lat_hs(~finished,:) = NaN_24_vec(~finished,:);
            % Lon_hs(~finished,:) = NaN_24_vec(~finished,:);
            flh_curr = fl_hrs(~finished);
            uw_curr = u_wind(~finished);
            vw_curr = v_wind(~finished);
            % alpha will remain unchanged during flt stp unless lands
            alpha_curr =  alpha(~finished);
            del_alpha_curr = del_alpha_dep(~finished);
            tw_curr = tail_w(~finished);
            cw_curr = cross_w(~finished);
            over_water_curr = over_water(~finished);
            lnd_wtr_ahd_curr = lnd_wtr_ahd(~finished);
            date_jul_curr = date_jul(~finished);
             
            % decln will (if tracked) be updated
            decln_curr = decln(~finished);
            
            % similarly to Locs, need hourly decln field to update for
            % early stops
            decln_curr_hs = NaN_24_vec(~finished); % 
            decln_hs(~finished,:) = NaN_24_vec(~finished,:);

            weib_str_exp_curr = weib_str_exp(~finished);
            weib_exp_curr = weib_exp(~finished);

            max_cur_flhs = max(ceil(flh_curr));

            for ih = 1:max_cur_flhs

                will_fly_ih = flh_curr > ih-1;
                % include fractional fl hrs in last hour
                fl_h_ih = max(min(1+flh_curr -ih,1),0);
                n_will_fly_h = sum(will_fly_ih);

               if include_wind
            
                    updt_wnds = ih > 1 && mod(ih,hrs_wnd_updat) == 1;
                    ...
                    if updt_wnds
                        ...
                        if wind_form == 1 % constant wind str
                            ...
                            [uw_curr, vw_curr] = ...
                                det_stoch_wind(theta(~finished),wind_str,NaN,NaN,0,true); % d
            
                        else % Weibull 
                
                            % Set is_uncorr to false since no within step
                            % stopovers
                            % And set is_wnd_swtch to true
                            % Need to div prev wind by w_str 
                            % due to dimensionless calcn Weibull
                            [uw_curr(will_fly_ih), vw_curr(will_fly_ih)] = ...
                                det_stoch_wind_weib(Lat_currs(will_fly_ih), ...
                                weib_str_exp_curr(will_fly_ih),weib_exp_curr(will_fly_ih), ...
                                opt_stdy_swtch_ws, ...
                                uw_curr(will_fly_ih)/wind_str, vw_curr(will_fly_ih)/wind_str, ...
                                false(n_will_fly_h,1),true,p_stdy_w_updt); % d
                            % scale wind to airsp (inverse)
                            uw_curr(will_fly_ih) = wind_str*uw_curr(will_fly_ih);
                            vw_curr(will_fly_ih) = wind_str*vw_curr(will_fly_ih);
            
                        end
            
                        % Determine tw and cw compts without
                        % any wind compn, waiting for TWs @coast
                        % Set is_deps to false
                        % Jan 2025: set last arg (mnt_TW_comp_wtr) to true!
                        % i.e.,retain offset to heading for wind drift each hour
                        % e.g., until next "fixing" at dawn
                           [tw_curr(will_fly_ih), cw_curr(will_fly_ih)] = ...
                           calc_wind_supprt_drft(alpha_curr(will_fly_ih), ...
                           uw_curr(will_fly_ih),vw_curr(will_fly_ih), N_S_migr, ...
                           false, over_water_curr(will_fly_ih), ...
                              lnd_wtr_ahd_curr(will_fly_ih), ...
                              false, false,false, false, NaN,del_alpha_curr,true);
                        % adjust for comp at cst
                        % alpha = alpha + del_alpha_dep;
               ...
               % update locn for current (one) hour of flt
               [Lat_currs(will_fly_ih), Lon_currs(will_fly_ih), ...
                    ~,~,decln_curr(will_fly_ih)] = ...
                    update_loc_hrly(fl_h_ih(will_fly_ih),hourly_del_Lat, ...
                Lat_currs(will_fly_ih),Lon_currs(will_fly_ih), ...
                alpha_curr(will_fly_ih),N_S_migr, ...
                tw_curr(will_fly_ih), cw_curr(will_fly_ih),magn_star_night, ...
                decln_curr(will_fly_ih), ...
                date_jul_curr(will_fly_ih),geomagn_model,1);

               Lat_curr_hs(will_fly_ih,ih) = Lat_currs(will_fly_ih);
               Lon_curr_hs(will_fly_ih,ih) = Lon_currs(will_fly_ih);
               decln_curr_hs(will_fly_ih,ih) = decln_curr(will_fly_ih);

               if ih > 1
                   Lat_curr_hs(~will_fly_ih,ih) = Lat_curr_hs(~will_fly_ih,ih-1);
                   Lon_curr_hs(~will_fly_ih,ih) = Lon_curr_hs(~will_fly_ih,ih-1);
                   decln_curr_hs(~will_fly_ih,ih) = decln_curr_hs(~will_fly_ih,ih-1);
               end

            end

            for ih = max_cur_flhs+1:24
                Lon_curr_hs(:,ih) = Lon_curr_hs(:,max_cur_flhs);
                Lat_curr_hs(:,ih) = Lat_curr_hs(:,max_cur_flhs);
                decln_curr_hs(:,ih) = decln_curr(:,max_cur_flhs);
            end

            % try

            % Lat_fins(~finished,:) = Lat_currs;
            % Lon_fins(~finished,:) = Lon_currs;
            Lat_hs(~finished,:) = Lat_curr_hs;
            % Lon_hs(~finished,:) = Lon_curr_hs;           
            Lon_hs(~finished,:) = ~near_dateline*Lon_curr_hs + ...
                near_dateline*mod(Lon_curr_hs,2*pi); 

            decln(~finished) = decln_curr;
            decln_hs(~finished,:) = decln_curr_hs;

            % now set up check for arrival on coasts

            fl_hrs(finished) = 0;

            idx_cst_hs = near_idx_cst(mod(Lon_hs(~finished,:)*rad2deg, 360), ...
                    Lat_hs(~finished,:)*rad2deg);
           
            % not_nan_idx = find(~isnan(idx_cst_hs));
            % if hrzn_opt

            d_csts_hs = d_cst_vec(idx_cst_hs); % d_cst_vec(idx_cst_hs(not_nan_idx));

            over_wtr_hs(~finished,:) = d_csts_hs<0; % not_nan_idx
           ...
           %  end
            ...
            % Check options for stopping (current flight) at coast
            % If was over water at flight beginning, wants to stop immediately
            stop_nxt_land(~finished) = ...
                curr_over_wtr(~finished) == 1 & stpWtrFlt == 1; % & cum_hrs_flt(~finished) > 2*max_fl_stp_hs
            stop_nxt_land(finished) = false;

            % Next determine number of extra (searching) flight hours to
            % look, if over water, for coast / elevn land
            
            fracStop = stop_nxt_land.*(fracStopCst*(fl_hrs==24) + ...
                0*(fl_hrs~=24)) + ...
                ~stop_nxt_land.*(~init_endur*fracStopCst + ...
                init_endur*fracStopCst_init); %  

            nx_fl_hs = min(floor(fracStop.*fl_hrs/10),23);
            % dx_fl_hs = fracStop*fl_hrs - nx_fl_hs;

            min_fl_hs = max(nx_fl_hs,1);

            will_lnd_erly = ~finished & (stop_nxt_land | stopCstOpt); % ...      

            rem_fl_hs = max(ceil(fl_hrs) - nx_fl_hs,1); % nx_fl_hs.*will_lnd_erly;
            curr_fl_hs = will_lnd_erly.*min_fl_hs + ...
                ~will_lnd_erly.*ceil(fl_hrs);

            % try
            lin_cur_flh = sub2ind(size(Lon_hs), 1:size(Lon_hs,1), max(curr_fl_hs,1)')';  

            
            is_over_wtr = over_wtr_hs(lin_cur_flh);
            % idx_over_wtr = find(is_over_wtr);
  
            % catch
            %     keyboard
            % end

            % set locs to that of last obligate flight hour
            llamda(~finished) = Lon_hs(lin_cur_flh(~finished))';
            theta(~finished) = Lat_hs(lin_cur_flh(~finished))';

            decln(~finished) = decln_hs(lin_cur_flh(~finished))';

            if any(isnan(llamda(~finished)))
                find(isnan(llamda) & ~finished,2,'first')
                keyboard
            end


            lat_deg_idx(~finished,1) = 91 + floor(-180/pi*theta(~finished));
            lon_deg_idx(~finished,1) = min(360,floor(180/pi*llamda(~finished)) + 181);

            is_high_elev = hi_elevs(sub2ind([180 360], ...
                lat_deg_idx,lon_deg_idx));
       
            is_barrn = barrens(sub2ind(size(barrens), ...
                min(lat_deg_idx,size(barrens,1)),lon_deg_idx));
            
            % remove polar cases for land or ocean compatibility
             lost = lost | (abs(theta) > lost_Lat*deg2rad);
             finished = finished | lost;                         

            % now update hourly candidate locations if lands at coast 

            needs_find_lnd = ~finished & will_lnd_erly & is_over_wtr; % & 
            idx_wants_to_lnd = find(needs_find_lnd);
 
            fl_hs_done = ~needs_find_lnd;
            idxfh_done = find(fl_hs_done);
            
            for ii = 1:sum(fl_hs_done)

                ii_fhd = idxfh_done(ii);
                min_flh_pl = min_fl_hs(ii_fhd)+1;

                if ~finished(ii_fhd) % o/w unchanged
                                    
                    theta(ii_fhd) = Lat_hs(ii_fhd,min_flh_pl-1);
                    Lat_hs(ii_fhd,min_flh_pl:24) = ...
                        theta(ii_fhd);
                    llamda(ii_fhd) = Lon_hs(ii_fhd,min_flh_pl-1);
                    Lon_hs(ii_fhd,min_flh_pl:24) = ...
                        llamda(ii_fhd);

                else

                    % theta(ii_fhd) = NaN;
                    Lat_hs(ii_fhd,min_flh_pl:24) = NaN;
                    % llamda(ii_fhd) = NaN;
                    Lon_hs(ii_fhd,min_flh_pl:24) = NaN;                  

                end
                
            end

            % initially idx points to all th_wl_s, ll_wl_s (changes as birds
            % land)
            % idx_not_yet_landed = idx_wants_to_lnd;
            idx_loc_nrs = (1:numel(idx_wants_to_lnd))';

            fls_done = false(size(idx_wants_to_lnd)); % fl_hs_done(idx_not_yet_landed);

            % 
            % ow_wls = over_wtr_hs(idx_wants_to_lnd,:);

            if ~any(~fl_hs_done) 
                idx_not_yet_landed = [];
            end
                        
            while any(~fl_hs_done) 

               idx_loc_nrs = idx_loc_nrs(~fls_done);
               idx_not_yet_landed = idx_wants_to_lnd(idx_loc_nrs);

                Lon_wl_hs = Lon_hs(idx_not_yet_landed,:);
                Lat_wl_hs = Lat_hs(idx_not_yet_landed,:);
      
                %% in case we need to find linear idx from 2-d array
                ...
                curr_fls = curr_fl_hs(idx_not_yet_landed);
                rem_fls = rem_fl_hs(idx_not_yet_landed);
                fls_done = fl_hs_done(idx_not_yet_landed);
                           
                % won't be visible outside max_rad_stop_dawn degrees
                % Lat and *1/cos(Lat) degs Lon 

                lin_wl_flh = sub2ind(size(Lon_wl_hs), 1:size(Lon_wl_hs,1), max(curr_fls,1)')';  
                
                % set locs to that of last obligate flight hour
                th_wl_s = Lat_wl_hs(lin_wl_flh);
                ll_wl_s = Lon_wl_hs(lin_wl_flh);
                ll_wl_s = ~near_dateline*ll_wl_s + ...
                    near_dateline*mod(ll_wl_s,2*pi);  

                % th_wl_s = theta(idx_not_yet_landed);
                % ll_wl_s = ~near_dateline*llamda(idx_not_yet_landed) + ...
                %     near_dateline*mod(llamda(idx_not_yet_landed),2*pi);  
           
                if ~hrzn_opt % can create poly entire popn

                % for hrzn opt more efficient to define per indivl

                   mn_qnt_see_cst = 0; % 0.05; % 0.01;  
                   mx_qnt_see_cst = 1; % 0.95; % 0.99;  

                    min_lt = quantile(th_wl_s,mn_qnt_see_cst); % min(th_wl_s);
                    max_lt = quantile(th_wl_s,mx_qnt_see_cst); % max(th_wl_s);

                    lat_poly_land = [min_lt-max_rad_stop_dawn ...
                        max_lt+max_rad_stop_dawn max_lt+max_rad_stop_dawn ...
                        min_lt-max_rad_stop_dawn min_lt-max_rad_stop_dawn NaN];
    
                     min_ln = quantile(ll_wl_s,mn_qnt_see_cst); % min(ll_wl_s);
                     max_ln = quantile(ll_wl_s,mx_qnt_see_cst); % max(ll_wl_s);

                    max_dLon_dawn = ...
                        max_rad_stop_dawn/cos(max(abs(min_lt),abs(max_lt)));
    
                    lon_poly_land = [min_ln-max_dLon_dawn ...
                        min_ln-max_dLon_dawn  max_ln+max_dLon_dawn ...
                        max_ln+max_dLon_dawn min_ln-max_dLon_dawn NaN];

                    poss_vis = inpolygon(coastlon,coastlat, ...
                        lon_poly_land,lat_poly_land);
                    poss_lt_land = coastlat(poss_vis);
                    poss_ln_land = coastlon(poss_vis);

                end
                  
                   n_to_land = numel(idx_not_yet_landed);

                   lnd_succ = false(n_to_land,1);
                   lnd_hs = NaN(n_to_land,1);
                   fin_Lts = NaN(n_to_land,1);
                   fin_Lns = NaN(n_to_land,1);

                  for i_nl = 1:n_to_land % par

                       % need to point to original wants to land indices
                       % based on which individuals (idx_loc_nrs) are still
                       % flying
                       % idx_nl = idx_loc_nrs(i_nl); % idx_wants_to_lnd(i_nl); % 

                       th_wl_i = th_wl_s(i_nl);
                       ll_wl_i = ll_wl_s(i_nl);

                       % idx_iow = idx_not_yet_landed(idx_nl);
                        if hrzn_opt

                            % poss_lt_land = NaN;
                            % poss_ln_land = NaN;

                            hrzn_th = (th_wl_i-max_rad_stop_dawn):hrzn_grid_res_rad:(th_wl_i+max_rad_stop_dawn);
                            max_dll_wl_i = max_rad_stop_dawn/cos(th_wl_i);
                            hrzn_ll = (ll_wl_i-max_dll_wl_i):hrzn_grid_res_rad:(ll_wl_i+max_dll_wl_i);
                            % [hrzn_th_gr,hrzn_ll_gr] = meshgrid(hrzn_th,hrzn_ll);
                            [hrzn_th_gr,hrzn_ll_gr] = ndgrid(hrzn_th,hrzn_ll);
                            % idx_hrzn_land = near_idx_hrzn(hrzn_ll_gr, ...
                            %         hrzn_th_gr); 
                            idx_hrzn_land = near_idx_hrzn(hrzn_th_gr, ...
                                    hrzn_ll_gr); 
                            d2_hrzn_land = d2_hrzn(idx_hrzn_land(:)); %

                            d2_gt_0 = d2_hrzn_land>0;

                            poss_lt_land = hrzn_th_gr(d2_gt_0);
                            poss_ln_land = hrzn_ll_gr(d2_gt_0);
                            d2_hrzn_land = d2_hrzn_land(d2_gt_0);                        

                        end

                        % replace d2cst by Cartesian (local planar) distance 
                        % since within bird's eye view spherical effects
                        % are neglible

                         d_SeeCst_2s = (th_wl_i-poss_lt_land).^2 + ...
                            (cos(th_wl_i)*(ll_wl_s(i_nl) -poss_ln_land)).^2; % sqrt()

                        if ~hrzn_opt % quadr decreasing chance detectn with dist

                            p_dtc = ...
                                min(max((1-d_SeeCst_2s/max_rad_st_dwn_2),0),1); 

                            if sum(p_dtc) > 0
                            
                                nonz = find(p_dtc ~=0);
                                % sees_cst = nonz(p_dtc(nonz) >= ...
                                %     rand(size(d_SeeCsts(nonz))));
                                sees_cst = nonz(p_dtc(nonz) >= ...
                                    rand(size(d_SeeCst_2s(nonz))));
                        ...
                        else % use d2_hrzn eg for LB Cuckoo           
                             
                            sees_cst = find(d_SeeCst_2s < d2_hrzn_land ...
                                & rand(size(d_SeeCst_2s)) <  p_see_lnd_pixl); % Inf); %

                        end

                        % clear d2_hrzn_land d2_gt_0 d_SeeCst_2s


%                 try
                        % create temp vars to facilitate parfor loop
                        % temp_ow = ow_wls(idx_nl,curr_fls(i_nl):24);
                        % temp_Lt_hs = Lat_wl_hs(idx_nl,curr_fls(i_nl):24); 
                        % temp_Ln_hs = Lon_wl_hs(idx_nl,curr_fls(i_nl):24); 

                     n_sees = numel(sees_cst);
                     if n_sees > 0 
                         
                         % ows(idx_nl) = 0;

                         %  w_ds = 1./d_SeeCst_2s(sees_cst);
                         % idx_sees = randsample(sees_cst,1,true,w_ds);

                         idx_sees = sees_cst((d_SeeCst_2s(sees_cst) ...
                             == min(d_SeeCst_2s(sees_cst))));              

                         % avoid choosing same (1st) land point
                         % rnd_land = randi(numel(idx_closest));

                         % update locn to nearest coast
                         % theta(idx_iow) = poss_lt_land(idx_closest(rnd_land));
                         % llamda(idx_iow) = poss_ln_land(idx_closest(rnd_land));
                         fin_Lts(i_nl) = poss_lt_land(idx_sees(1));
                         fin_Lns(i_nl) = poss_ln_land(idx_sees(1));
                         % fin_Lns(i_nl) = shiftAnglesFromMinus180To180(fin_Lns(i_nl)*rad2deg)*deg2rad;
                         % fin_Lns(i_nl) = shiftAnglesFromMinus180To180(ll_wl_s(i_nl)*rad2deg)*deg2rad;

                         %% check if idx ok
                         % rem_fl_hs(idx_not_yet_landed(idx_nl)) = 0;

                         % fl_hs_done(idx_iow) = true;
                         fls_done(i_nl) = true;   

                         % try

                         % update hourly fields for arrival criteria
                         % (in case they already passed land in 
                         % arrival area before leaving it)
                         lnd_succ(i_nl) = true;
                         lnd_hs(i_nl) = curr_fls(i_nl);

                         % fin_Lts(i_nl) = th_wl_s(i_nl);
                         % fin_Lns(i_nl) = ll_wl_s(i_nl);
                         % catch
                         %     keyboard
                         % end

                         % idx_not_yet_landed = idx_not_yet_landed([1:idx_nl-1 idx_nl+1:end]);

                     else

                            %% add update of hours here??
                            % rem_fl_hs(idx_iow) = rem_fl_hs(idx_iow) -1;
                            % curr_fl_hs(idx_iow) = curr_fl_hs(idx_iow) +1;
                            % 
                            % fl_hs_done(idx_iow) = rem_fl_hs(idx_iow) == 0;                                
                            ...
                            if rem_fls(i_nl) > 0
                                ...
                                % try
                                % update flight hour 
                                 rem_fls(i_nl) = rem_fls(i_nl) -1;
                                 curr_fls(i_nl) = curr_fls(i_nl) +1;

                                 % update hourly location
                                 % th_wl_s = theta(idx_not_yet_landed(i_nl),curr_fls(i_nl));
                                 %    ll_wl_s = ~near_dateline*llamda(idx_not_yet_landed(i_nl),curr_fls(i_nl)) + ...
                                 %        near_dateline*mod(llamda(idx_not_yet_landed(i_nl),curr_fls(i_nl)),2*pi);  
                                % th_wl_s(i_nl) = Lat_wl_hs(i_nl,curr_fls(i_nl));
                                % ll_wl_s(i_nl) = Lon_wl_hs(i_nl,curr_fls(i_nl));   
                                % catch
                                %     keyboard
                                % end
                   ...
                   succ_hs = lnd_hs(lnd_succ);
                   idx_succ = find(lnd_succ);

                   % convert subscripts to linear indices
                   % idx_succ = sub2ind([numel(succ_hs) 24], ...
                   %     repmat(lnd_succ,[1 24]), ...
                   %     succ_hs:24);
                   % ow_wls(~lnd_succ,succ_hs:24) = true;

                   for isc = 1:numel(succ_hs)

                       % try
                
                       idx_sc_i = idx_succ(isc);
                       idx_lc_i = idx_not_yet_landed(idx_sc_i);
                       llamda(idx_lc_i) = fin_Lns(idx_sc_i);
                       theta(idx_lc_i) = fin_Lts(idx_sc_i);

                       if isnan(fin_Lns(idx_sc_i))
                           keyboard
                       end

                       % replicate final locn until end of day
                       Lat_hs(idx_lc_i,succ_hs(isc):24) = fin_Lts(idx_sc_i ); 
                       Lon_hs(idx_lc_i,succ_hs(isc):24) = fin_Lns(idx_sc_i ); 

                       over_wtr_hs(idx_lc_i,succ_hs(isc):24) = false;
                       over_water(idx_lc_i) = false;
                   ...
                   % here those not wanting to land early will be unchanged
                   % fl_hrs(idx_not_yet_landed) = curr_fls;
                   curr_fl_hs(idx_not_yet_landed) = curr_fls;
                   rem_fl_hs(idx_not_yet_landed) = rem_fls;
                   fl_hs_done(idx_not_yet_landed) = fls_done; 

                   % over_water(idx_not_yet_landed(~fls_done)) = true;
                   % over_water(idx_not_yet_landed(fls_done)) = false;
                   % over_water(isnan(rem_fl_hs)) = true;
                   % 
                   % catch
                   % 
                   %     keyboard
             ...
             % update final (nightly) flight-step hours
            fl_hrs = curr_fl_hs;
            ...
            % update final locations of non-landed inds
            % idx_fail = find(~lnd_succ);
            % idx_lc_f_i = idx_not_yet_landed(idx_fail);
            % llamda(idx_lc_f_i) = fin_Lns(idx_fail);
            % theta(idx_lc_f_i) = fin_Lts(idx_fail);

            if ~isempty(idx_not_yet_landed)
                 llamda(idx_not_yet_landed) = fin_Lns;
                 theta(idx_not_yet_landed) = fin_Lts;
            end
            % if any(llamda ~= Lon_hs(:,end) & ~finished)
            %     keyboard
            % end

            % if any(theta > pi) || any(blat_hs(:) > pi)
            %     keyboard 
            % end          
            ...
            % over_wtr_hs(idx_not_yet_landed,end) = true;

           % update candidates for landing
           % 
           % over_wtr_hs(idx_wants_to_lnd,:) = ow_wls;

           % det idx
           %  Lon_hs(idx_wants_to_lnd,:) = Lon_wl_hs;
           %  Lat_hs(idx_wants_to_lnd,:) = Lat_wl_hs;
           % 
           % llamda(idx_wants_to_lnd) = ll_wl_s;
           % theta(idx_wants_to_lnd) = th_wl_s;
           ...
           % fl_hrs = curr_fl_hs;
            ...
            over_water(will_lnd_erly & rem_fl_hs == 0) = true;
            
%% update state (finsihed, survived etc
...
%             over wtr updated above
            is_high_elev = hi_elevs(sub2ind([180 360], ...
                lat_deg_idx,lon_deg_idx));
            is_barrn = ~over_water & barrens(sub2ind(size(barrens), ...
                min(lat_deg_idx,size(barrens,1)),lon_deg_idx));  
...
%                 is_barrn(~finished);
         ...
         % update_arrive_survive_lost_finished
            update_arr_surv_lost_fin_hs
            % this updates idx_not_fin
        ...
        if ~endur_flt || stpovCstWtr
            is_poor_stop = poor_stops(sub2ind(size(poor_stops), ...
                min(lat_deg_idx,size(poor_stops,1)),lon_deg_idx));
            skip_stop_over(~finished) = over_water(~finished) | is_high_elev(~finished) | ...
                is_poor_stop(~finished);
        else
            skip_stop_over(~finished) = true;   
        end
...
%         try
       ...
       if ~isempty(idx_not_fin)
           ...
           [ahead_theta, ahead_llamda] = update_loc(...
            ones(size(idx_not_fin)),hourly_del_Lat,theta(~finished), ... radAheadDtrCst/2
            llamda(~finished),alpha(~finished),N_S_migr, ...
            zeros(size(idx_not_fin)),zeros(size(idx_not_fin)), ...
           ...
           % try
            ...
            % 2024 added qualification TW selection only over land!!
            lnd_wtr_ahd(~finished) = d_ahd_csts < 0 & ~over_water(~finished);
           % catch
           % 
           %     keyboard
           % 
       ...
       % will stop if option pre wtr flight stopovers
        pre_wtr_stop = lnd_wtr_ahd & stpovCstWtr;

        % wants_continue_fly = (cumul_flts(~finished) <= n_flts_seq -1);
        % Apr 2022 added possibility to tank after 1/2 of max fl hrs used
        % up. Also new option stpovWtrFlt to stop over
        ...
        wants_continue_fly =  ...
            ...
            ~(stop_nxt_land(~finished) & stpovWtrFlt) & ...
            ~pre_wtr_stop(~finished);  %
...
%             & curr_max_ns_fl_hs(~finished)
        ...
        stops_over(~finished) = ~skip_stop_over(~finished) & (~wants_continue_fly | ...
            req_stpovr_poly | pre_wtr_stop(~finished) | req_stpovr_sgnp);
...
%        end
        ...
        % 
           ...
           % if any(stop_num > 3 & blats*180/pi > -30)
           % 
           %     keyboard
           % 
           % end
...
% try
            ...
            reg_stop = stops_over & ~is_new_sgnp & ~pre_wtr_stop;
            N_reg_stp = sum(reg_stop);
            cst_stop = stops_over  & ~is_new_sgnp & pre_wtr_stop;
            N_cst_stp = sum(cst_stop);
            sgnp_stop = stops_over & is_new_sgnp;
            N_sgnp_stp = sum(sgnp_stop);
        ...
        if N_stop_overs > 0
            ...
            ndvi_stop_csts = ...
             ndvi(sub2ind([180 360],lat_deg_idx(cst_stop), ...
             lon_deg_idx(cst_stop)));              

             ndvi_stop_sgnps = ...
             ndvi(sub2ind([180 360],lat_deg_idx(sgnp_stop), ...
             lon_deg_idx(sgnp_stop))); 

             rnd_veg_fr = rand(size(one_vec));

             lat_idx_trunc = min(sz_vg(1),lat_deg_idx);

             veg_fr_stp = rnd_veg_fr.* ...
                 veg_stp_md(sub2ind(sz_vg,lat_idx_trunc,lon_deg_idx)) + ...
                 (1-rnd_veg_fr).* ...
                 veg_stp_95(sub2ind(sz_vg,lat_idx_trunc,lon_deg_idx));
             veg_fr_sum = rnd_veg_fr.* ...
                 veg_stp_sum_md(sub2ind(sz_vg,lat_idx_trunc,lon_deg_idx)) + ...
                 (1-rnd_veg_fr).* ...
                 veg_stp_sum_95(sub2ind(sz_vg,lat_idx_trunc,lon_deg_idx));

             veg_fr_prox = veg_fr_stp.*(theta < 37.5*pi/180) + ...
                 veg_fr_sum.*(theta >= 37.5*pi/180);

             % stopgap - use jja for stops in Europe

             veg_stop_regs = veg_fr_prox(reg_stop);
             veg_stop_csts = veg_fr_prox(cst_stop);
             veg_stop_sgnps = veg_fr_prox(sgnp_stop);

            % frac_ndvi_reg = min((ndvi_stop_regs - thrsh_ndvi_stop_min)/ndvi_rng,1); 
            % frac_ndvi_cst = min((ndvi_stop_csts - thrsh_ndvi_stop_min)/ndvi_rng,1); 
            % frac_ndvi_sgnp = min((ndvi_stop_sgnps - thrsh_ndvi_stop_min)/ndvi_rng,1); 

            frac_veg_reg = max(min((veg_stop_regs - min_any_veg_stp)/veg_rng,1),0); 
            frac_veg_cst = max(min((veg_stop_csts - min_any_veg_stp)/veg_rng,1),0); 
            frac_veg_sgnp = max(min((veg_stop_sgnps - min_any_veg_stp)/veg_rng,1),0); 
            frac_DFL_reg = frac_veg_reg; % frac_ndvi_reg; %.*  
            frac_DFL_cst = frac_veg_cst; % frac_ndvi_cst; %
            frac_DFL_sgnp = frac_veg_sgnp; % frac_ndvi_sgnp; % .*
...
%             curr_max_ns_fl_hs(stops_over)-cum_hrs_flt(stops_over)); 
            ...
            if any(isnan(frac_veg_reg)) || any(isnan(frac_veg_cst)) || ...
                    any(isnan(frac_veg_sgnp))
           ...
           curr_max_ns_fl_hs(reg_stop) = ...
            min(max(frac_DFL_reg.*rand([N_reg_stp 1]).* ...
           (max_ns_hs_bs(reg_stop) - min_ns_hs_bs(reg_stop)) + ...
           min_ns_hs_bs(reg_stop),curr_max_ns_fl_hs(reg_stop) - ...
           cum_hrs_flt(reg_stop)+min_ns_hs_bs(reg_stop)),max_ns_hs_bs(reg_stop)); % Inf); % 

           % curr_max_ns_fl_hs(cst_stop) = ...
           %  min(max(frac_DFL_cst.*rand([N_cst_stp 1]).* ...
           % (max_cst_hs_bs(cst_stop) - min_cst_hs_bs(cst_stop)) + ...
           % min_cst_hs_bs(cst_stop), curr_max_ns_fl_hs(cst_stop) - ...
           % cum_hrs_flt(cst_stop)),max_cst_hs_bs(cst_stop)); % Inf); 

            curr_max_ns_fl_hs(cst_stop) = ...
            min(max(frac_DFL_cst.*rand([N_cst_stp 1]).* ...
           (max_cst_hs_bs(cst_stop) - min_cst_hs_bs(cst_stop)) + ...
           min_cst_hs_bs(cst_stop), curr_max_ns_fl_hs(cst_stop) - ...
           cum_hrs_flt(cst_stop)+min_ns_hs_bs(cst_stop)),max_cst_hs_bs(cst_stop)); % Inf); %

           curr_max_ns_fl_hs(sgnp_stop) = ...
            min(max(frac_DFL_sgnp.*rand([N_sgnp_stp 1]).* ...
           (max_sgnp_hs_bs(sgnp_stop) - min_sgnp_hs_bs(sgnp_stop)) + ...
           min_sgnp_hs_bs(sgnp_stop),curr_max_ns_fl_hs(sgnp_stop) - ...
           cum_hrs_flt(sgnp_stop)+min_ns_hs_bs(sgnp_stop)),max_sgnp_hs_bs(sgnp_stop)); % Inf); %
        ...
        if tc_comp_wtr == 1
            ...
            d_time = sign(d_time_zone).*floor(abs(d_time_zone))  + ...
                ~finished.*(~stops_over + (reg_stop | cst_stop)*stop_durn + sgnp_stop*stop_durn_sp);
...
% try
    ...
    % if TCSC, calculate flight hours next time step
...
%       TCSC clock shift (which is subsequently set to zero as bird resets
%       its inner clock
%                    
           ...
           rand_fact = 1 + rand_wtr_fact*(over_water | is_barrn);
...
%                 d_alpha_geo = (~finished).*zero_vec;
                ...
                alpha_perfects{1} = alpha_bases{1} + decln;
                ...
                for i_comp = 1:3
                    if  calibr_comp(i_comp) > 0 
                       ...
                       alpha_err_all = [alpha_err_all d_alpha_errs{i_comp}];
                    end
                end

                % new 2024 add adjustemnt for comp cst (e.g., for over
                % water flight, possibly maintained next day over wtr if 
                % mnt_TW_comp_wtr = true
                alpha_all = alpha_all + del_alpha_dep;

%             ...
%                             && cal_comp_nonzero(i_comp)
                 % can add compensation for prev detour (when on land)
                 alpha_mean = circ_mean(alpha_all')';
                 alpha_err_mean = circ_mean(alpha_err_all')';
...
%             end
               ...
               alpha(reorients & ~finished) = mod( ...
                   ...
                   (reOrWtrStr==7).*(alpha_lst_lnd(reorients & ~finished) ... 
                    ...
                    alpha_err_mean(reorients & ~finished) + ...
               ...
               %% new preferred heading has now been established
                % next, calculate winds, determine tailwinds and
                % check and adjust heading for any detours or wind compn

                % first check for detours
               if dtrCstOpt==1

                    idx_cst_0 = near_idx_cst(theta*rad2deg,llamda*rad2deg);
                    % end
                    d_csts_all = d_cst_vec(idx_cst_0);
                    % over_water_0 = d_csts_0 < 0;
                   
                   % d2cst = dist2coast(theta*rad2deg,llamda*rad2deg);
                   can_dtr = ~finished & ~over_water & ...
                       (dtrCstOpt==1)*(d_csts_all > 0 & ...
                       d_csts_all<kmsAheadDtrCst) & ...
               ...
               recmp_dtr = reorients | cum_steps_dtr  > 0;
                ...
                % next, test for updated winds; this may be
                ...
                if include_wind
                   ...
                   if wind_form == 1 % constant wind str
                        ...
                        det_stoch_wind(theta(~finished),wind_str, ...
                        u_wind(~finished), v_wind(~finished),opt_stdy_swtch_ws,true); %d
        
                    else % Weibull

                    % differentiate between stop over and transient
                    % (or begin in air) steps
    
                     % need to first rescale previous winds
                     % as Weibull most easily scaled to mean w sp in m/s
                         [u_wind(~finished), v_wind(~finished)] = ...
                             det_stoch_wind_weib(theta(~finished), ...
                             weib_str_exp(~finished),weib_exp(~finished), ...
                            opt_stdy_swtch_ws, ...
                            u_wind(~finished)/wind_str, v_wind(~finished)/wind_str, ...
                            stops_over(~finished),true,p_stdy_w_updt); % d
                                         % scale wind to airsp (inverse)
                        u_wind(~finished) = wind_str*u_wind(~finished);
                        v_wind(~finished) = wind_str*v_wind(~finished);

                       % [uw_curr(will_fly_ih), vw_curr(will_fly_ih)] = ...
                       %          det_stoch_wind_weib(Lat_currs(will_fly_ih), ...
                       %          weib_str_exp,weib_exp,opt_stdy_swtch_ws, ...
                       %          uw_curr/wind_str, vw_curr/wind_str, ...
                       %          false(n_will_fly_h,1),true,p_stdy_w_updt); % d
        
                    end

                    % [u_wind(~finished), v_wind(~finished)] = ...
                    % det_stoch_wind_weib(theta(~finished),weib_str_exp, ...
                    % u_wind(~finished), v_wind(~finished),opt_stdy_swtch_ws); % 
                ...
                % if include_wind % == 1
                %     tail_w(~finished) = calc_wind_supprt(alpha(~finished), ...
                %         u_wind(~finished), v_wind(~finished), N_S_migr);
                % end

               % work out detours for those who did not begin but will be over
               % water 

               %% finally increment flight step and arrival to 
               % places of interest e.g. EU for wheatears
          ...
          if Sim_nr > N_init_Sims && iYear == N_mig_Sims % - 2 % 1 % 
              
            annotate_fields
 
          end

          % for any later steps set Lat_hs Lon_hs to NaNs
          Lat_hs(finished,:) = NaN_24_vec(finished,:);
          Lon_hs(finished,:) = NaN_24_vec(finished,:);
    ...
    %               mn_lon_Deps(iA,iYear) = circ_mean(llamda_0(is_iA))*180/pi;                    
        ...
        succ_Dom = Dom_phen(stoppedAndArrived);

       if Sim_nr <=  N_init_Sims 

            succ_wrmp(iWarmup) = N_succ/N_inds; 
           ...
           if N_S_migr
                 ...
                 all_ys_inher_heads(:,iYear) = inher_heads*180/pi;
           end

           % if ~isinf(n_chrs)
                all_ys_allels(:,:,:,iYear) = all_allels;
           % else
           %     all_ys_allels(:,:,:,iYear) = all_allels;
           % end
...
%                 coeff_succ = all_coeffs(stoppedAndArrived,:); 
             ...
             if zug_opt == 1  
                 ...
                 if N_S_migr
                    ...
                    all_ys_zugkn_heads(:,iYear) = rad2deg*zugkn_inher_heads;
                 end
      ...
      %% if last generation (year), save all field variables
          if iYear == N_mig_Sims && iWarmup > N_init_Sims % -1
             ...
             if N_S_migr
                 ...
                 succ_heads = rad2deg*succ_heads;
                 all_inher_heads = rad2deg*inher_heads;
                 all_zugkn_heads = rad2deg*zugkn_inher_heads;
                 succ_zug_inh_hds = rad2deg*succ_zug_inh_hds; 
             end
