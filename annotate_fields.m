...
blat_hs = [blat_hs Lat_hs];
blon_hs = [blon_hs Lon_hs];
...
if N_S_migr
    ...
    all_geo_heads = [all_geo_heads mod(alpha*rad2deg,360)];
    all_mag_heads = [all_mag_heads mod((alpha - decln)*rad2deg,360)];
    all_sun_az = [all_sun_az sun_az*180/pi];
end
