...
% Maximum duration
max_mig_durs = [110 120 140 140 90 90 50 70 100, 70, 70, 30];
...
fracStopCst = (mig_sys ~= 12)*7  + (mig_sys == 12)*4; % 7; % 5 %   mig_sys ~= 4*7 + mig_sys == 4*10; % 1; % 
...
stpWtrFlt = 1*(mig_sys ~= 12); %  0 % (mig_sys ~= 4)*1 % 0; %
stpovWtrFlt =  0 %  1; %   (mig_sys ~= 4)*1 % 0; %
% stop at coast when water is ahead 
stpovCstWtr = 1*(mig_sys ~= 12); % 0; %
...
if incl_wind
   ...
   %     (mig_sys==10 || mig_sys == 11)*0.15; %  + (mig_sys==12)*0.1;
    ...
    prompt_TWs = {'Wait TWs depart?'};
    TWs_list = {'no','wait at cst','always wait'};
    ...
    prompt_wind_dtl2s = {'Compens HWs deps (0/1)?', ...
        'compens HWs @cst? (0/1)', 'compens TWs deps? (0/1)',
        'compens TWs @cst? (0/1)','Fract comp (0-1)?', ...
        'maintain comp over wtr? (0/1)'};
...
tic
    ...
    [successful, succ_wrmp, succs_DepPs, succs_ArrPs, succs_DepArrPs, ...
     ...
     simul_comp_rtes_hrzn(n_loci,n_chrs,geo_filnm,N_inds,N_mig_Sims,N_wrmp_rnd, ...
      ...
      stopCstOpt,fracStopCst,stpWtrFlt,stpovWtrFlt,stpovCstWtr, ...
      ...
      hrzn_opt,hrzn_grid_res,fl_alt_lev);
toc
