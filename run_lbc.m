...
% prompt_loci = {'# genes inher dirn?'};
% n_loci = listdlg('PromptString',prompt_loci,'ListString',{'1','2'}); 

n_loci = 1;

% prompt_rnd_allel = {'Init allel distr?'};
rndm_init_allels = 4; % listdlg('PromptString',prompt_rnd_allel, ...
...
if n_loci == 2
    ...
    n_chrs = listdlg('PromptString',prompt_chrs,'ListString',{'1','2'}); 
    ...
    EW_dom = false; % listdlg('PromptString',prompt_EW,'ListString',{'360-deg','E/W'}) == 2; 
...
% Maximum duration
max_mig_durs = [110 120 140 140 90 90 50 70 100, 70, 70, Inf];
...
%     fieldsize,definput);
thrsh_all_veg_stop =  0.5; % 0.05; % str2double(thr_veg_cel{1});
% % additive factor to error when over water
min_any_veg_stp = 0.0; % str2double(thr_veg_cel{2}); % 0.5; %  1; % 
...
Arr_verts_mig_sys = {{[-70 -50 -10 0]}, ...
    ...
    {[154 168 -23 5],[168 182 -23 5],[182 196 -23 5],[196 210 -23 5]}}; % [154 168 -23 5], , , [210 224 -23 5] [150 215 -23 5] {[160 168 -23 -5], [178 215 -23 -5]}}; %[-20 5 0 15][-20 0 60 70] [169 184 -40 -33]};[30 50 -10 15]
...
% Option to stop prematurely over coast when over water on
% arrival e.g. at dawn
...
if incl_wind
    ...
    prompt_wind_dtls = {'Mean windsp (m/s)', ...
        'Stoch HW TW? (0/1)','Max wsp all deps (0/1)?', ...
        'Max wsp @cst (m/s)?'};
    definput = {'7','1','Inf','Inf'};
   ...
   %     (mig_sys==10 || mig_sys == 11)*0.15; %  + (mig_sys==12)*0.1;
    ...
    switch_wind_opt = strcmp(wind_dtl_str{2},'1');
    ...
    dep_TWs = false;
    switch_wind_opt =  false; % (incl_wind==1); %
...
prompt_stds = {'Max ntl disp (km)', 'Max wint conn (km)','std hds (deg)', ...
    'std incln (deg)','std  int (%)'};
definput = {'25','Inf','5','1','.01'};
fieldsize = [1 45; 1 45; 1 45; 1 45; 1 45];
dlgtitle = 'Input';
std_str = inputdlg(prompt_stds,dlgtitle, ...
    fieldsize,definput);
max_disp_d_dep_km = str2double(std_str{1});
max_disp_d_arr_km = str2double(std_str{2});
max_std_inh_hd = str2double(std_str{3});
max_std_inh_sp = (zug_opt == 1)*str2double(std_str{4}) + ...
    (zug_signp == 3)*str2double(std_str{5});
...
if ~isinf(max_disp_d_arr_km)
    min_disp_d_arr_km = min_disp_d_dep_km; %5*d_min; %d_min; %   + ~is_whtr*2*d_min; % d_min; %20;
...
tic
    ...
    [successful, succ_wrmp, succs_DepPs, succs_ArrPs, succs_DepArrPs, ...
       ...
       all_ys_allels, all_ys_inher_heads, all_ys_init_geo_heads, ...
     ...
     simul_comp_rtes_hrzn(n_loci,n_chrs,geo_filnm,N_inds,N_mig_Sims,N_wrmp_rnd, ...
      ...
      incl_wind,wind_str,wind_form,weib_scale,weib_par,switch_wind_opt, ...
      ...
      frc_cmp_cst,mnt_TW_comp_wtr, max_wsp_dep,max_wsp_cst, ...
      hrzn_opt,hrzn_grid_res,fl_alt_lev,p_see_lnd_pixl);
toc
