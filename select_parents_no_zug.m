...
%      end
    ...
    allel_hd_new_1(i_dep,1) = succ_al_hds(i_parnt_1(i_dep),r_t1);
    allel_hd_new_2(i_dep,1) = succ_al_hds(i_parnt_2(i_dep),r_t2);

    % allel_hd_new_1(i_dep,1) = circ_mean([allel_hd_new_1(i_dep,1)' allel_hd_new_1(i_dep,2)'])';
    % allel_hd_new_2(i_dep,1)  = allel_hd_new_1(i_dep,1);
    % if zug_opt == 1 %
    %     for iz = 1:n_zugs % add one to allow parfor loop 
    %         % (won't run anyway if n_zug == 0)
    %         allel_zg_new_1(i_dep,iz) = succ_al_zgs(i_parnt_1(i_dep),r_t1,iz);
    %         allel_sp_new_1(i_dep,iz) = succ_al_sps(i_parnt_1(i_dep),r_t1,iz);
    %         allel_zg_new_2(i_dep,iz) = succ_al_zgs(i_parnt_2(i_dep),r_t2,iz);
    %         allel_sp_new_2(i_dep,iz) = succ_al_sps(i_parnt_2(i_dep),r_t2,iz);    
    %     end
    % end
    end
