function [successful, succ_wrmp, succs_DepPs, succs_ArrPs, succs_DepArrPs, ...
  ...
  simul_comp_rtes_genl ...
    ...
    maxDtrSteps,stopCstOpt,fracStopCst,stpWtrFlt,stpovWtrFlt,stpovCstWtr, ...
    ...
    wt_natal_intns,wt_natal_incln,wt_natal_decln,opt_same_loc_brd, ...
    accel_wrmp,arr_EU_opt,veg_mod_pal, ...
    match_init_Lons,n_heads,EW_dom,EW_avg,genSel,genSelWt,rndm_init_allels, ...
    unif_allels,comp_HWs,comp_TWs_cst,hrzn_opt,fl_alt_lev)
...
%             curr_head = alpha;
            ...
            [theta(~finished), llamda(~finished)] = update_loc(...
                ...
                uw_eff(~finished), vw_eff(~finished),magn_star_night, ...
            ...
            % if ~near_dateline
            %     idx_cst = near_idx_cst(llamda(~finished)*rad2deg, ...
            %         theta(~finished)*rad2deg);
            % else
            idx_cst = near_idx_cst(mod(llamda(~finished)*rad2deg, 360), ...
                    theta(~finished)*rad2deg);

            % end
            d_csts = d_cst_vec(idx_cst);
            over_water(~finished) = d_csts<0;
% try
...
% check if initial step of endurance flight (then won't stop for visible land)
            ...
            init_endur = n_fl_step==1 & endur_flt;
...
%             end
            ...
            will_land_early = ~finished &  ~init_endur & stop_nxt_land; % ...
                % (stop_nxt_land | over_water); % (stop_nxt_land |over_water | is_high_elev | is_barrn)

            % over_wtr_fracs = ~island(thetas_fracs*rad2deg, ...
            %     llamdas_fracs*rad2deg); %

            % if ~near_dateline
            %     idx_cst_fr = near_idx_cst(llamdas_fracs*rad2deg, ...
            %         thetas_fracs*rad2deg);
            % else
                
            idx_cst_fr = near_idx_cst(mod(llamdas_fracs*rad2deg,360), ...
                    thetas_fracs*rad2deg);

            if hrzn_opt

                idx_hrzn_fr = near_idx_hrzn(mod(llamdas_fracs*rad2deg, 360), ...
                    thetas_fracs*rad2deg); 

                d_hrzn_frs = d_hrzn(idx_hrzn_fr);

            end

            % end
            d_csts = d_cst_vec(idx_cst_fr);
            over_wtr_fracs = d_csts<0;
...
%                 over_wtr_fracs(will_land_early,:) = ~island(lat_will_land_fr, ...
                ...
                is_barrn_fr = barrens(sub2ind(size(barrens),min(lat_fr_idx,size(barrens,1)),lon_fr_idx));
...
%              over_wtr_fracs(~will_land_early,10) = 0;
            ...
            idx_wants_lnd = find(will_land_early);
             ...
             if (any(n_fl_step>1) && stopCstOpt) || sum(will_land_early) > 0
...
%                 over_water(~finished) = over_water(~finished) & land_frac(~finished) == 10;
                ...
                idx_over_wtr = find(over_water & ~finished & ~init_endur);
...
%                     ow_10s = ones(size(idx_over_wtr));
                    ...
                    ll_ws = ~near_dateline*llamda(idx_over_wtr) + ...
                        near_dateline*mod(llamda(idx_over_wtr),2*pi);   
                    ...
                    max_dLon_dawn = max_rad_stop_dawn/cos(max(abs(min_lt),abs(max_lt)));

                    lat_poly_cst = [min_lt-max_rad_stop_dawn ...
                    ...
                    lon_poly_cst = [min_ln-max_dLon_dawn ...
                    ...
                    cst_poss_lt = coastlat(poss_vis);
                    cst_poss_ln = coastlon(poss_vis);
...
%                             ll_ws(i_ow),coastlat,coastlon,'radians'); % , ang_Stops
                        ...
                        else % use d_hrzn eg for LB Cuckoo

                              p_dtc = 1*(d_SeeCst_2s < d_hrzn_frs); % some function

                        end

                        % p_dtc = ...
                        %     max((1-d_SeeCst_2s/max_rad_st_dwn_2),0); 

%                         sees_cst = find(p_dtc >= rand(size(d_SeeCsts)));
%                          sees_cst = p_dtc >= rand(size(d_SeeCsts));
                        if sum(p_dtc) > 0
...
%                          p_dtc = zeros(size(d_SeeCsts));
%                          p_dtc(d_SeeCsts<max_rad_stop_dawn)
...
%                 try
                         if numel(sees_cst) > 0 
                             ...
                             ows(i_ow) = 0;
%                              ow_10s(i_ow) = 0;
                             % idx_closest = sees_cst(d__SeeCsts(sees_cst) ...
                             %     == min(d_Stops(sees_cst)));
                             idx_closest = sees_cst(d_SeeCst_2s(sees_cst) ...
...
%                              idx_closest = find(d_Stops == min(d_Stops),1,'first');
        %                                  alpha(n_fl_step+1) = ang_Stops(idx_closest)*deg2rad-pi;
                             ...
                             th_ws(i_ow) = cst_poss_lt(idx_closest(1));
                             ll_ws(i_ow) = cst_poss_ln(idx_closest(1));
...
%         try
       ...
       if ~isempty(idx_not_fin)
           ...
           % ahead_wtr = island(ahead_theta*rad2deg, ahead_llamda*rad2deg) == 0;
            ...
            % end
            d_ahd_csts = d_cst_vec(idx_ahd_cst);
            ahead_wtr(~finished) = d_ahd_csts < 0;
       ...
       % will stop if option pre wtr flight stopovers
        pre_wtr_stop = ahead_wtr & stpovCstWtr;
...
%             curr_max_ns_fl_hs(stops_over)-cum_hrs_flt(stops_over)); 
           ...
           min_ns_hs_bs(reg_stop), ...
            ...
            max(frac_DFL_cst.*rand([N_cst_stp 1]).* ...
           (max_cst_hs_bs(cst_stop) - min_cst_hs_bs(cst_stop)) + ...
           min_cst_hs_bs(cst_stop), ...
            curr_max_ns_fl_hs(cst_stop)-cum_hrs_flt(cst_stop)); 

           curr_max_ns_fl_hs(sgnp_stop) = ...
            max(frac_DFL_sgnp.*rand([N_sgnp_stp 1]).* ...
