function [successful, succ_wrmp, succs_DepPs, succs_ArrPs, succs_DepArrPs, ...
  ...
  simul_comp_rtes_genl ...
    ...
    include_wind,wind_str,wind_form,weib_scale,weib_par,switch_wind_opt, ...
    ...
    frc_cmp_cst,mnt_TW_comp_wtr,max_wsp_dep,max_wsp_cst,hrzn_opt,hrzn_grid_res,fl_alt_lev,p_see_lnd_pixl)
       ...
       % lnd_wtr_ahd = island(ahead_theta*rad2deg, ahead_llamda*rad2deg) == 0;
        ...
        if include_wind
            ...
            if wind_form == 1 % constant wind str
                ...
                [u_wind, v_wind] = det_stoch_wind(theta,wind_str,NaN,NaN,0); % d

            else % Weibull 
    
                [u_wind, v_wind] = det_stoch_wind_weib(theta,weib_str_exp,NaN,NaN,0,weib_exp); % d
...
%         arr_EU = false_vec;
        ...
        while sum(~finished) > 0 % 0.001*N_inds
           ...
           %  end
            ...
            will_lnd_erly = ~finished &  ... % ~init_endur & 
                (stop_nxt_land | stopCstOpt); % ...      
            ...
            is_barrn = barrens(sub2ind(size(barrens),min(lat_deg_idx,size(barrens,1)),lon_deg_idx));
            ...
            wants_lnd_erly = is_over_wtr & will_lnd_erly;
            idx_wants_to_lnd = find(wants_lnd_erly);
 
            fl_hs_done = ~wants_lnd_erly;
            ...
            while any(~fl_hs_done) 
               ...
               idx_not_yet_landed = idx_wants_to_lnd(idx_loc_nrs);
                ...
                lin_cur_flh = sub2ind(size(Lon_wl_hs), 1:size(Lon_wl_hs,1), max(curr_fls,1)')';  
                
                % set locs to that of last obligate flight hour
                th_wl_s = Lat_wl_hs(lin_cur_flh);
                ll_wl_s = Lon_wl_hs(lin_cur_flh);
...
%             over wtr updated above
            is_high_elev = hi_elevs(sub2ind([180 360],lat_deg_idx,lon_deg_idx));
            is_barrn = ~over_water & ...
                barrens(sub2ind(size(barrens),min(lat_deg_idx,size(barrens,1)),lon_deg_idx));  
...
%                 d_alpha_geo = (~finished).*zero_vec;
                ...
                % new 2024 add adjustemnt for comp cst (for over
                % water flight, possibly maintained next day over wtr if 
                % mnt_TW_comp_wtr = true
                alpha_all = alpha_all + del_alpha_wtr;
               ...
               recmp_dtr = reorients | cum_steps_dtr  > 0;
                ...
                if include_wind
                   ...
                   if wind_form == 1 % constant wind str
                        ...
                        u_wind(~finished), v_wind(~finished),switch_wind_opt); %d
                     ...
                     % as Weibull most easily scaled to mean w sp in m/s
                         [u_wind(~finished), v_wind(~finished)] = ...
                             det_stoch_wind_weib(theta(~finished),weib_str_exp(~finished), ...
                        u_wind(~finished)/wind_str, v_wind(~finished)/wind_str,switch_wind_opt,weib_exp(~finished)); % d
                                         % scale wind to airsp (inverse)
                    ...
                    % u_wind(~finished), v_wind(~finished),switch_wind_opt); % 
