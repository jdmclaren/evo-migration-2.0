...
% succ_traits = NaN*ones(numel(idx_succ),2);
% succ_allels(:,:,1) = circ_mean([all_allels(idx_succ,1,1)'; all_allels(idx_succ,2,1)'])';
succ_allels = NaN*ones(numel(idx_succ),2,n_loci);
succ_allels(:,:,1) = all_allels(idx_succ,:,1);

% assume alleles are homozygous for trait inhertance
if n_loci > 1
    succ_allels(:,:,2) = all_allels(idx_succ,:,1);
% else
...
% instead of alleles use traits ie heads, zugs and sps
succ_al_hds = [inher_heads(idx_succ,:) inher_heads(idx_succ,:)]; % allel_heads
succ_al_zgs = [zugkn_inher_heads(idx_succ,:,:) zugkn_inher_heads(idx_succ,:,:)]; % allel_zugs 
succ_al_sps = [zug_signps(idx_succ,:,:) zug_signps(idx_succ,:,:)]; % allel_sps
