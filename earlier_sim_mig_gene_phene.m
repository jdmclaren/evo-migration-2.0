function [successful, succ_wrmp, succs_DepPs, succs_ArrPs, succs_DepArrPs, ...
    ...
    arrived, survived, lost, stoppedAndArrived, all_ys_allels, ...
...
%     prev_coeff_succ = []; 
    ...
    if n_chrs == 1 || isinf(n_chrs)
        ...
        all_allels = NaN(N_inds_sim,2,n_loci); % ,min(n_loci,2+2*n_zugs))n_req;
        allel_heads = NaN(N_inds_sim,n_loci,n_loci);
        allel_zugs = NaN(N_inds_sim,n_loci,n_loci,n_zugs);
        allel_sps = NaN(N_inds_sim,n_loci,n_loci,n_zugs);
    end
...
%         arr_EU = false_vec;
        ...
        while sum(~finished) > 0 % 0.001*N_inds
                    ...
                    if updt_wnds
                        ...
                        % Set is_deps to false
                           ...
                           false, over_water_curr(will_fly_ih), ...
                              ...
                              false, false,false, false, NaN,del_alpha_curr,mnt_TW_comp_wtr);
                        % adjust for comp at cst
                        % alpha = alpha + del_alpha_dep;
            ...
            min_fl_hs = max(floor(fl_hrs) - nx_fl_hs,1);

            will_lnd_erly = ~finished & (stop_nxt_land | stopCstOpt); % ...      

            rem_fl_hs = nx_fl_hs.*will_lnd_erly;
            curr_fl_hs = will_lnd_erly.*min_fl_hs + ...
                ~will_lnd_erly.*fl_hrs;
...
% try
        ...
        if N_stop_overs > 0
            ...
            ndvi_stop_csts = ...
             ...
             lat_idx_trunc = min(size(sz_vg,1),lat_deg_idx);
...
%             curr_max_ns_fl_hs(stops_over)-cum_hrs_flt(stops_over)); 
           ...
           cum_hrs_flt(reg_stop)+min_ns_hs_bs(reg_stop)),max_ns_hs_bs(reg_stop)); 
           ...
           cum_hrs_flt(cst_stop)+min_ns_hs_bs(cst_stop)),max_cst_hs_bs(cst_stop)); 
           ...
           cum_hrs_flt(sgnp_stop)+min_ns_hs_bs(sgnp_stop)),max_sgnp_hs_bs(sgnp_stop)); 
