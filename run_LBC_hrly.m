...
migr_str = {'NAm SAm ','N Eur Afr ', 'Palearctic Afr ', 'Alk NZ ', ...
    ...
    'Pan Wheatear','Blackpoll West','Blackpoll East','LBCuckoo'};
...
% prompt_loci = {'# genes inher dirn?', '(Inf = phenotyp inher)'};
% n_loci = listdlg('PromptString',prompt_loci,'ListString',{'1','2','Inf'}); 

n_loci = 3; 

if n_loci == 1  % isinf(n_loci)
    ...
    % n_chrs = listdlg('PromptString',prompt_chrs,'ListString',{'1','Inf'}); 
    % if n_chrs == 2
    %     n_chrs = Inf;
    %     n_loci = 1;
    %     n_heads = 1;
...
if n_loci >= 2
    ...
    % unif_allels = false;
    % 
    % prompt_rnd_allel = {'Init Rand allels?'};
    % rndm_init_allels = listdlg('PromptString',prompt_rnd_allel,'ListString',{'no','yes'}) ==2;  
    % if rndm_init_allels
    %     rndm_init_all_str = 'RndInitAll';
    % else
    %     rndm_init_all_str = 'nonRndInitAll';
    ...
    genSel = false; %  listdlg('PromptString',prompt_genSel,'ListString',{'no','yes'}) ==2;  
    % 
    % if genSel
    % 
    ...
    % prompt_chrs = {'N Chromas encoding hds?'};
    n_chrs = 3; % listdlg('PromptString',prompt_chrs,'ListString',{'1','2','Inf'}); 
    if n_chrs == 3
        n_chrs = Inf;
        n_loci = 1;
        n_heads = 1;
    ...
    elseif n_chrs == 2
        ...
        prompt_nHds = {'E/W ctrl or' , '360-deg hds?'};
        n_heads = listdlg('PromptString',prompt_nHds,'ListString',{'E/W','360'}); 
        EW_dom = n_heads == 1;
    end

    % prompt_match_Lon = {'Bias init heads per', 'Longs Deps & Arrs?'};
    match_Lon_nr = 1; % listdlg('PromptString',prompt_match_Lon,'ListString',{'No','by Lon','by gene'});        
    match_init_Lons = false; % match_Lon_nr - 1; % match_Lon_nr == 2;
    
else

    % n_chrs = 1;

    prompt_EW = {'360-deg hds or' , 'E/W ctrl?'};
    EW_dom = listdlg('PromptString',prompt_EW,'ListString',{'360-deg','E/W'}) == 2; 
...
% if EW_dom 
% 
%     prompt_EW = {'Head only (E/W) dom?'};
%     EW_avg = listdlg('PromptString',prompt_EW,'ListString',{'Always avg','Dom head'}); 
% 
% else
...
endg_cmp = 1; % listdlg('PromptString',prompt_inh,'ListString',comp_list);
endog_comp =  (endg_cmp==1)*3 + (endg_cmp==2)*1 % 3 %  

% prompt_prim = {'Choose primary compass'};
prim_cmp = 1; % listdlg('PromptString',prompt_prim,'ListString',comp_list); 
calibr_comp =  [prim_cmp==2 0 prim_cmp==1];

% prompt_star = {'Choose in-flight compass'};
star_cmp = 1; % listdlg('PromptString',prompt_star,'ListString',comp_list); 
...
% thr_veg_cel = inputdlg(prompt_thr_veg,dlgtitle, ...
    % fieldsize,definput);
thrsh_all_veg_stop = 0.5; %  str2double(thr_veg_cel{1});
% % additive factor to error when over water
min_any_veg_stp = 0.05; % str2double(thr_veg_cel{2}); % 0.5; %  1; % 
...
if mig_sys ~= 12
    daily_surv = 0.999; % 1; % is_whtr*0.99 + ~is_whtr*1; %  0.98 %95; %  is_whtr* ~is_whtr*1 + 0.995; %  0.9975; 0.99; % 
    daily_surv_elev = 1; % 0.99; %  0.99; % 0.5; %  0.99;
    daily_surv_barrn = 0.995; %~is_whtr*0.99 + is_whtr*1; %; % 1; %  0.95; % 
    daily_surv_wtr = 0.995; %  1; % 
else
...
% fieldsize = [1 45; 1 45];
% dlgtitle = 'Input';
cst_opts_cel = {'0','0'}; % inputdlg(prompt_step_cst,dlgtitle, ...
    % fieldsize,definput);
dtrCstOpt = 0; % str2double(cst_opts_cel{1});
% % additive factor to error when over water
reOrWtr_opt = 0; % str2double(cst_opts_cel{2}); %
...
if incl_wind
   ...
   %     (mig_sys==10 || mig_sys == 11)*0.15; %  + (mig_sys==12)*0.1;
    ...
    definput = {'0','0','0','0','0','0'}; % '1','1','0.5','0'};
    fieldsize = [1 45; 1 45; 1 45; 1 45; 1 45; 1 45];
    dlgtitle = 'Input';
...
min_ns_cst_hs = min(min_ns_fl_hs*1.5,min_ns_sgnp_hs);
max_ns_cst_hs = min(max_ns_fl_hs*2,max_ns_sgnp_hs);
